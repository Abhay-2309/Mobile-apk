...
class BleMeshManager(private val context: Context, private val bluetoothAdapter: BluetoothAdapter?) {
    ...
    init {
        scanner.setOnPacketDiscoveredListener { rawBytes, rssi ->
    ...
    fun broadcastSos(
        ...
        val encoded = BlePacketCodec.encode(packet)
        log("SOS", "Creating SOS packet A3F7C2E1 (MessageId: 0x${messageId.toString(16).uppercase()}), TTL=$ttl, HOPS=$hopCount")

        val success = advertiser.startAdvertising(encoded)
        if (success) {
            log("BLE", "Started advertising SOS packet (MessageId: 0x${messageId.toString(16).uppercase()})")
        } else {
    ...
    private fun handleDiscoveredPacket(rawBytes: ByteArray, rssi: Int) {
        val packet = BlePacketCodec.decode(rawBytes)
        if (packet == null) {
        ...
        val encodedRelay = BlePacketCodec.encode(relayPacket)

        // Schedule burst advertising of forwarded packet
        Handler(Looper.getMainLooper()).post {
            log("BLE", "Forwarding packet $hexId over BLE advertising...")
